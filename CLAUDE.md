...
- `lib/booking` — `buildInquiryMailtoUrl()` and `buildInquiryTelUrl()` are the live "make a booking" implementations, used by the reservations page; it's commission-free for the guest and gets the best rate, so the page leads with it; the property's Airbnb and Vrbo listings (`ListingLink` in `components/property`) are offered below the rates table as secondary paths at higher rates. Future direct booking + Airbnb/VRBO calendar sync will live in this module when built (surface described in `docs/architecture.md`).
